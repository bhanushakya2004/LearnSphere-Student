...
import React, { useEffect, useState } from "react";
import Cookies from "js-cookie";
import { BookOpen, Plus, X, BookOpenCheck, GraduationCap, User, Layout, Search, LogOut, ChevronDown } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle, CardFooter, CardDescription } from "@/components/ui/card";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
...
const Dashboard = () => {
  ...
  // Check if user is navigating back to login/signup and handle cookies
  useEffect(() => {
    const handleBackNavigation = () => {
      window.onpopstate = () => {
        const pathName = window.location.pathname;
        if (pathName === "/login" || pathName === "/signup" || pathName === "/") {
          // Delete cookies if navigating back to auth pages
  ...
  useEffect(() => {
    const fetchStudentData = async () => {
      ...
      const email = Cookies.get("studentEmail");
      if (!email) {
        // Redirect to login if no email cookie exists
        navigate("/login");
        setLoading(false);
        return;
      }
  ...
  const handleLogout = () => {
    // Remove all cookies related to the user session
    Cookies.remove("studentEmail");
    Cookies.remove("selectedClassId");

...
const ClassPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Get student name from students collection if userEmail exists
        if (userEmail) {
    ...
    fetchData();
  }, [classId, firestore, toast, userEmail]);
    ...
    try {
      ...
      if (userEmail) {
        const submissionQuery = query(
          collection(firestore, 'submission'),
          where('assignmentId', '==', assignment.id),
          where('studentId', '==', userEmail)
        );
        
        const submissionSnapshot = await getDocs(submissionQuery);
        setAlreadySubmitted(!submissionSnapshot.empty);
      }
    } catch (error) {
  ...
  const handleSubmitText = async (e) => {
    e.preventDefault();
    if (!selectedAssignment || !userEmail || !classId) {
      toast({ title: "Error", description: "Invalid assignment, user, or class." });
      return;
    }
    ...
    try {
      ...
      await addDoc(collection(firestore, 'submission'), {
